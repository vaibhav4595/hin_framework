...
def ip_laplace(ngram, type):
	...
	if type == 0:
		x1 = l1*1.0*(transition.get( (wn_0, wn_1, wn_2) , 0) + kk)/(bigram.get( (wn_0, wn_1) , 0) + kk*len(unigram.keys()) )
		x2 = l2*1.0*(bigram.get( (wn_1, wn_2) , 0) + kk)/(unigram.get(wn_1, 0) + kk*len(unigram.keys()) )
	else:
		x1 = l1*1.0*(emission.get( (wn_0, wn_1, wn_2) , 0) + kk)/(bigram.get( (wn_0, wn_1) , 0) + kk*len(V) )
		x2 = l2*1.0*(wgram.get( (wn_1, wn_2) , 0) + kk)/(unigram.get(wn_1, 0) + kk*len(V))
	return x1 + x2
...
# 	prob = 1.0*(dictionary.get(tgram, 0) + k)/(bigram.get(bgram, 0) + k*total)
# 	return prob
...
if __name__ == "__main__":
	...
	#print sentences[3]
	#viterbi(transition, emission, sentences[3])
